...
var salaryHighCharts = {
    ...
    yearSalary:function (){
        salaryHighCharts.config.series = [];    
        var data = new Array();    
        data = salaryHighCharts.getData();    
        for(i=0; i < salaryHighCharts.profession.length;i++){
            var seriesOption = {
                name:salaryHighCharts.profession[i],
                data:data[i]
            };    
            salaryHighCharts.config.series.push(seriesOption);
        }                    
        Highcharts.chart('demo',salaryHighCharts.config);
    },
    yearGrowthRate:function(){
        salaryHighCharts.config.series = [];
        var growthRate = new Array();
        var data = salaryHighCharts.getData();
